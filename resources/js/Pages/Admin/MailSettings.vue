...
<template>
    <ConfigLayout section="correo">
        <div class="mb-5">
            <h2 class="flex items-center gap-2 text-lg font-semibold"><Mail class="h-5 w-5 text-brand" /> Correo</h2>
            <p class="text-sm text-slate-500">Proveedor de envío (SMTP o Mailgun), correo de prueba y eventos recientes.</p>
        </div>

        <!-- Configuración del proveedor -->
        <form class="space-y-4" @submit.prevent="submit">
            <div class="rounded-xl border border-slate-200 bg-white p-5 dark:border-slate-700 dark:bg-slate-800">
                <p class="mb-3 flex items-center gap-2 text-sm font-semibold"><Mail class="h-4 w-4 text-slate-400" /> Proveedor</p>
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    <div>
                        <label :class="label">Tipo de envío <span class="text-red-600">*</span></label>
                    ...
                    <div></div>
                    <div>
                        <label :class="label">Dirección de envío <span class="text-red-600">*</span></label>
                    ...
                    <div>
                        <label :class="label">Nombre remitente <span class="text-red-600">*</span></label>
                        <input v-model="form.from_name" :class="input" placeholder="Sistema META" />
            ...
            <div v-if="form.provider === 'mailgun'" class="rounded-xl border border-slate-200 bg-white p-5 dark:border-slate-700 dark:bg-slate-800">
                <p class="mb-3 flex items-center gap-2 text-sm font-semibold"><Cloud class="h-4 w-4 text-slate-400" /> Mailgun <span class="font-normal text-slate-400">· servicio transaccional vía API HTTP</span></p>
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    <div>
                    ...
                    <div>
                        ...
                        <input v-model="form.mg_secret" type="password" autocomplete="off" :class="input" :placeholder="settings.has_mg_secret ? '•••••••• (guardada — escribe para reemplazar)' : 'key-…'" />
                    </div>
                    <div>
                        <label :class="label">Clave de firma del webhook</label>
                        <input v-model="form.mg_webhook_key" type="password" autocomplete="off" :class="input" :placeholder="settings.has_mg_webhook ? '•••••••• (guardada)' : 'opcional'" />
                    </div>
                </div>
            </div>
            ...
            <div v-else class="rounded-xl border border-slate-200 bg-white p-5 dark:border-slate-700 dark:bg-slate-800">
                ...
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    <div>
                        <label :class="label">Servidor (host)</label>
                    ...
                    <div>
                        ...
                        <input v-model="form.smtp_password" type="password" autocomplete="new-password" :class="input" :placeholder="settings.has_smtp_password ? '•••••••• (guardada — escribe para reemplazar)' : ''" />
                    </div>
                    <div>
                        <label :class="label">Cifrado</label>
            ...
            <!-- Webhook -->
            <div class="rounded-xl border border-slate-200 bg-white p-5 dark:border-slate-700 dark:bg-slate-800">
                <p class="mb-1 flex items-center gap-2 text-sm font-semibold"><Webhook class="h-4 w-4 text-slate-400" /> Webhook de eventos</p>
                <p class="text-xs text-slate-500">Configura esta URL en Mailgun para recibir entregas, rebotes y aperturas en los logs del sistema.</p>
        ...
        <form class="mt-6 rounded-xl border border-slate-200 bg-white p-5 dark:border-slate-700 dark:bg-slate-800" @submit.prevent="sendTest">
            <p class="mb-3 flex items-center gap-2 text-sm font-semibold"><Send class="h-4 w-4 text-slate-400" /> Enviar correo de prueba <span class="font-normal text-slate-400">· guarde la configuración antes de probar</span></p>
            <div class="space-y-3">
                <div>
                    <label :class="label">Destinatario <span class="text-red-600">*</span></label>
                ...
                <div>
                    ...
                    <input v-model="testForm.subject" :class="input" placeholder="[META] Correo de prueba" />
                </div>
                <div>
                    <label :class="label">Mensaje</label>
                    <textarea v-model="testForm.message" rows="3" :class="input" placeholder="Texto plano. Si los campos quedan vacíos, se usarán los valores por defecto." />
                </div>
            </div>
            <div class="mt-4 flex justify-end">
                <button type="submit" :disabled="testForm.processing" class="inline-flex items-center gap-1.5 rounded-lg bg-brand px-4 py-2 text-sm font-medium text-white hover:opacity-90 disabled:opacity-50">
                    <Send class="h-4 w-4" /> {{ testForm.processing ? 'Enviando…' : 'Enviar prueba' }}
                </button>
            </div>
        </form>
        ...
        <div class="mt-6 rounded-xl border border-slate-200 bg-white dark:border-slate-700 dark:bg-slate-800">
            <p class="border-b border-slate-200 px-5 py-4 text-sm font-semibold dark:border-slate-700">Eventos recientes de correo <span class="font-normal text-slate-400">· últimos 50 recibidos por el webhook</span></p>
            <div class="overflow-x-auto">
                <table class="w-full text-sm">
                    ...
                    <tbody>
                        ...
                        <tr v-if="!events.length"><td colspan="4" class="px-5 py-8 text-center text-slate-400">Aún no se han recibido eventos de correo.</td></tr>
                    </tbody>
                </table>
            </div>
        </div>
