    ...
    <div class="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-black/40 p-6">
        <div class="w-full max-w-2xl rounded-2xl bg-white p-6 shadow-xl dark:bg-slate-800">
            <div class="flex items-start justify-between">
                <div>
                    <h2 class="text-xl font-semibold">{{ isEdit ? t('kpi_form.title_edit') : t('kpi_form.title_create') }}</h2>
                    <p class="mt-1 text-sm text-slate-500">{{ t('kpi_form.form_intro_before') }}<span class="text-red-600">*</span>{{ t('kpi_form.form_intro_after') }}</p>
                </div>
            ...
            <form class="mt-5 grid grid-cols-1 gap-4 md:grid-cols-2" @submit.prevent="submit">
                <div class="md:col-span-2">
                    <label :class="label">{{ t('kpi_form.label_name') }} <span class="text-red-600">*</span></label>
                    <input v-model="form.label" :class="input" :placeholder="t('kpi_form.placeholder_name')" />
                    <p v-if="form.errors.label" class="mt-1 text-xs text-red-600">{{ form.errors.label }}</p>
                </div>

                <div>
                    <label :class="label">{{ t('kpi_form.label_key') }} <span class="text-red-600">*</span></label>
                    <input v-model="form.key" :class="input" placeholder="ejecucion_presupuestaria" />
                    <p class="mt-1 text-xs text-slate-400">{{ t('kpi_form.key_hint') }}</p>
                ...
                <div>
                    <label :class="label">{{ t('kpi_form.label_current_value') }} <span class="text-red-600">*</span></label>
                ...
                <div>
                    <label :class="label">{{ t('kpi_form.label_target') }}</label>
                ...
                <div>
                    <label :class="label">{{ t('kpi_form.label_trend') }} <span class="text-red-600">*</span></label>
                    <select v-model="form.trend" :class="input">
                        <option value="" disabled>{{ t('kpi_form.trend_placeholder') }}</option>
                ...
                <div>
                    <label :class="label">{{ t('kpi_form.label_sort') }}</label>
                ...
                <div class="mt-2 flex justify-end gap-2 md:col-span-2">
                    ...
                    >
                        <Save class="h-4 w-4" /> {{ t('kpi_form.save_button') }}
                    </button>
                </div>
            </form>
        </div>
